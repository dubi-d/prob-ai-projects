...
class BayesianLayer(torch.nn.Module):
    ...
    def forward(self, inputs):
        # TODO: enter your code here
        epsilon = torch.normal(torch.zeros(self.weight_mu.shape),torch.ones(self.weight_mu.shape)) #needs shape of weight_mu
        weight = epsilon*torch.exp(self.weight_logsigma) + self.weight_mu

        if self.use_bias:
            eps = torch.normal(torch.zeros(self.bias_mu.shape),torch.ones(self.bias_mu.shape)) #needs shape of weight_mu
            bias = eps*torch.exp(self.bias_logsigma) + self.bias_mu
        else:
            bias = None
        ...
        # inputs = [128, 784]
        # weights = [784, 100]
        outputs = torch.matmul(inputs,weight) + bias #matrix mult.
    ...
    def _kl_divergence(self, mu, logsigma):
        ...
        # TODO: enter your code here
        Klloss = torch.log(self.prior_sigma) - logsigma + (torch.exp(logsigma)**2 
            - self.prior_sigma**2 + (mu - self.prior_mu)**2)/(2*self.prior_sigma**2)
        kl = torch.sum(Klloss)
...
class BayesNet(torch.nn.Module):
    ...
    def __init__(self, input_size, num_layers, width):
        super().__init__()
        self.input_layer = torch.nn.Sequential(BayesianLayer(input_size, width),
                                           nn.ReLU())
        self.hidden_layers = [nn.Sequential(BayesianLayer(width, width),
                                    nn.ReLU()) for _ in range(num_layers)]
    ...
    def predict_class_probs(self, x, num_forward_passes=10):
        ...
        summation = torch.zeros(batch_size,10)

        for i in range (0,num_forward_passes):
            out = self.net(x)
            softmax = F.softmax(out, dim=1)
            summation = torch.add(summation,softmax)

        average = torch.div(summation, num_forward_passes)
        probs = average
    ...
    def kl_loss(self):
        ...
        #input layer
        mKLloss += self.input_layer[0].kl_divergence()
        
        #hidden layers
        for batchlayer, relulayer in self.hidden_layers:
            mKLloss += batchlayer.kl_divergence()
            
        # output layers    
        mKLloss += self.output_layer.kl_divergence()
            
        return mKLloss


def train_network(model, optimizer, train_loader, num_epochs=100, pbar_update_interval=100, batch_size=128):
    '''
...
def main(test_loader=None, private_test=False):
    num_epochs = 101 # You might want to adjust this
    batch_size = 32 # Try playing around with this
    print_interval = 100
    ...
    train_network(model, optimizer, train_loader,
                 num_epochs=num_epochs, pbar_update_interval=print_interval,batch_size=batch_size)
